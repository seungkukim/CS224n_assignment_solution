...
def naiveSoftmaxLossAndGradient(
    ...
    V, _ = outsideVectors.shape
    exp_center_sim = np.exp(centerWordVec.T @ (outsideVectors[outsideWordIdx]))
    exp_logits = np.exp(outsideVectors @ centerWordVec)
    y = np.zeros(V)
    y[outsideWordIdx] = 1
    y_pred = exp_logits / np.sum(exp_logits)
    # import ipdb ; ipdb.set_trace()
    loss = - np.log(exp_center_sim / np.sum(exp_logits))
    gradCenterVec = outsideVectors.T @ (y_pred - y)
    gradOutsideVecs = (y_pred - y)[:,np.newaxis] @ centerWordVec[np.newaxis,:]
...
def negSamplingLossAndGradient(
    ...
    outsideWordVec = outsideVectors[outsideWordIdx][:,np.newaxis]
    
    loss = - np.log(sigmoid(centerWordVec.T @ outsideWordVec)) - np.sum(np.log(sigmoid(-outsideVectors[negSampleWordIndices] @ centerWordVec)))

    gradCenterVec = (sigmoid(centerWordVec.T @ outsideWordVec) - 1).item() * outsideWordVec.squeeze()
    gradCenterVec += (outsideVectors[negSampleWordIndices].T @ (1 - sigmoid(- outsideVectors[negSampleWordIndices] @ centerWordVec))).squeeze()

    num_occurences = np.zeros((V, 1))
    for ix in negSampleWordIndices:
        num_occurences[ix] += 1

    gradOutsideVecs = num_occurences * (1 - sigmoid(- outsideVectors @ centerWordVec)) * centerWordVec.reshape(1, D)
    gradOutsideVecs[outsideWordIdx] += (sigmoid(centerWordVec.T @ outsideWordVec) - 1).item() * centerWordVec.squeeze()
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        w_loss, w_gradCenterVec, w_gradOutsideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += w_loss
