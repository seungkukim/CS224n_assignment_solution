...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_len = random.randint(4, min(len(doc), int(self.block_size * 7/8)))
        trunc_start = random.randint(0, len(doc) - trunc_len)
        trunc_inp = doc[trunc_start: trunc_start + trunc_len]

        mask_len = random.randint(0, int(trunc_len / 2))
        mask_start = random.randint(0, trunc_len - mask_len)
        prefix, masked_content, suffix = trunc_inp[:mask_start], trunc_inp[mask_start:mask_start+mask_len], trunc_inp[mask_start+mask_len:]
