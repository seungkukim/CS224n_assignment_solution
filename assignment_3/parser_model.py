...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_bias = nn.Parameter(torch.nn.init.uniform_(torch.empty(1, self.hidden_size)))

        dropout_idx = torch.rand(1, self.hidden_size) < self.dropout_prob 
        dropout_mx = torch.ones(1, self.hidden_size) / self.dropout_prob
        dropout_mx[dropout_idx] = 0
        self.dropout = nn.Parameter(dropout_mx)
        self.hidden_to_logits_weight = nn.Parameter(torch.nn.init.xavier_uniform_(torch.empty(self.hidden_size, self.n_classes)))
        self.hidden_to_logits_bias = nn.Parameter(torch.nn.init.uniform_(torch.empty(1, self.n_classes)))
